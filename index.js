...
app.get('/data', (req, res) => {
    ...
    let s = '';
    for (let i = 0; i < x.length; i++) { s += frontManager.putInTableModData(x[i]); };
    res.json({ "data": s });
    res.end();
});
...
app.post('/searchforentry', (request, response) => {
    ...
    else response.json({ "data": '<h1 id="search_box_header">No entry found</h1>' });
    response.end();
});
app.post('/searchforschedule', (request, response) => {
    ...
    if (result2 && result3) {
        let r = '';
        for (let u = 0; u < result3.length; u++) { r += frontManager.putInTable(result3[u]); }
        v = frontManager.scheduleEnumInd(result2) + r;
        response.json({ "data": v });
    } else response.json({ "data": `<h2 id="search_box_header">Schedule not found under name "${scname}"</h2>` });
    response.end();
});
app.post('/createschedule', (request, response) => {
    ...
    else response.json({ "data": '<h1 id="search_box_header">Error, Schedule already exists or bad input</h1>' });
    response.end();
});
app.post('/addtoschedule', (request, response) => {
    let worked = backManager.saveSchedule(sanitize.sanitize(request.body.value1), sanitize.sanitize(request.body.value2), sanitize.sanitize(request.body.value3));
    if (worked) response.json({ "data": '<h1 id="search_box_header">Success</h1>' });
    else response.json({ "data": '<h1 id="search_box_header">Could not save to schedule.  Courses do not exist or bad input.</h1>' });
    response.end();
});
app.post('/deleteschedule', (request, response) => {

    let worked = backManager.deleteSchedule(sanitize.sanitize(request.body.value));
    if (worked) response.json({ "data": '<h1 id="search_box_header">Succcess</h1>' });
    else response.json({ "data": '<h1 id="search_box_header">No schedules to delete under that input</h1>' });
    response.end()
});
app.post('/deleteschedules', (request, response) => {
    let worked = backManager.resetSchedules();
    if (worked) response.json({ "data": '<h1 id="search_box_header">Succcess</h1>' });
    else response.json({ "data": '<h1 id="search_box_header">No schedules to delete</h1>' });
    response.end()
});
